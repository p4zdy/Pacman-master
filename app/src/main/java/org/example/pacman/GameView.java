...
public class GameView extends View {
	...
	protected void onDraw(Canvas canvas) {
		...
		//draw the ghost
		canvas.drawBitmap(game.getGhostBitmap(), game.getGhostx(), game.getGhosty(), paint);
		//loop through the list of gold coins and draw them.
		for (GoldCoin coin : game.getCoins()) {
		    if(!coin.taken) {
                canvas.drawBitmap(game.getCoinBitmap(), coin.coinx, coin.coiny, paint);
            }
		}
